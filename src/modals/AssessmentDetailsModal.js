...
import AIReportEditModal from './AIReportEditModal';

export default function AssessmentDetailsModal({ visible, assessment, onClose, onGeneratePDF, onGenerateAI, generatingAI, onSaveAIReport, savingAIReport, onEdit, onDelete, theme }) {
    const [editAIVisible, setEditAIVisible] = useState(false);
    ...
    // 🔥 estado do diagnóstico por IA para essa avaliação 🔥
    ...
    return (
        <Modal visible={visible} transparent animationType="fade">
            <View style={styles.detailsOverlay}>
                <View style={[styles.detailsCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} bounces={false} overScrollMode="never">
                        ...
                        {/* 🔥 BOTÃO EDITAR DIAGNÓSTICO — só aparece se já existe um diagnóstico gerado 🔥 */}
                        {onSaveAIReport && hasAIReport && !generatingAI && (
                            <TouchableOpacity 
                                onPress={() => setEditAIVisible(true)} 
                                style={[styles.editAiButton, { borderColor: theme.border }]}
                            >
                                <MaterialCommunityIcons name="pencil-box-outline" size={18} color={theme.text} />
                                <Text style={[styles.editAiButtonText, { color: theme.text }]}>EDITAR DIAGNÓSTICO</Text>
                            </TouchableOpacity>
                        )}

                        {hasAIReport && !generatingAI && (
                            <Text style={[styles.aiGeneratedNote, { color: theme.textSecondary }]}>
                                Diagnóstico atualizado em {aiGeneratedLabel}.
                            </Text>
                        )}
            ...
            {/* 🔥 MODAL DE EDIÇÃO DO DIAGNÓSTICO IA 🔥 */}
            <AIReportEditModal
                visible={editAIVisible}
                assessment={assessment}
                onClose={() => setEditAIVisible(false)}
                onSave={async (payload) => {
                    const ok = await onSaveAIReport(payload);
                    if (ok) setEditAIVisible(false);
                }}
                saving={savingAIReport}
                theme={theme}
            />
        </Modal>
    );
...
const styles = StyleSheet.create({
    ...
    pdfButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#111', borderWidth: 1, borderRadius: 12, paddingVertical: 15, marginBottom: 12, gap: 10 },
    ...
    editAiButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderRadius: 10, paddingVertical: 10, marginBottom: 8, gap: 8 },
    editAiButtonText: { fontWeight: '900', fontSize: 11, letterSpacing: 0.5 },
