...
import { View, Text, TouchableOpacity, ScrollView, TextInput, Modal, Alert, Platform, Dimensions } from 'react-native';
...
export default function RaioxCargasModal({ visible, onClose, historicoDeCargasList, theme }) {
    ...
    // --- LÓGICA DE LAYOUT WEB (igual ao AdminDashboard) ---
    const webOuterBg = theme.isDark ? '#0a0a0a' : '#E5E5EA';

    // No mobile: bottom sheet. Na web: modal centralizado com maxWidth 960.
    const overlayStyle = isWebPC
    ...
    return (
        <Modal visible={visible} transparent animationType={isWebPC ? 'fade' : 'slide'} onRequestClose={onClose}>
            {/* Fundo externo: na web usa a mesma cor de fundo do AdminDashboard */}
            <View style={[overlayStyle, isWebPC && { backgroundColor: webOuterBg + 'CC' }]}>
                                                ...
                                                {chartData.length > 1 && !isCardio && (
                                                    <View style={{ marginBottom: 25, alignItems: 'center', backgroundColor: theme.surface, paddingVertical: 15, borderRadius: 12, borderWidth: 1, borderColor: theme.border }}>
                                                        ...
                                                        <LineChart
                                                            data={chartData}
                                                            height={120}
                                                            width={isWebPC ? 560 : 260}
                                                            spacing={isWebPC ? 80 : 55}
                                                            initialSpacing={20}
                                                            color={theme.accent}
                                                            thickness={3}
