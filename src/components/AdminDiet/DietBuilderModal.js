        ...
        <View style={{ flex:1 }}>
            ...
            <View style={{ paddingHorizontal:10, paddingTop:8, gap:6 }}>
                ...
                <View style={{ zIndex:100 }}>
                    ...
                    {catOpen && (
                        <View style={[styles.catDropList, { backgroundColor:theme.surface, borderColor:theme.border }]}>
                            <ScrollView style={{ maxHeight:180 }} showsVerticalScrollIndicator={false} nestedScrollEnabled>
                                {CATEGORIES.map(cat => (
                                    <TouchableOpacity key={cat}
                                        style={[styles.catDropItem, { backgroundColor: category===cat ? theme.accent+'18' : 'transparent', borderBottomColor:theme.border }]}
                                        onPress={() => { setCategory(cat); setCatOpen(false); setResults([]); }}
                                    >
                                        <MaterialCommunityIcons name={category===cat ? 'radiobox-marked' : 'radiobox-blank'} size={14} color={category===cat ? theme.accent : theme.textSecondary} />
                                        <Text style={{ color: category===cat ? theme.accent : theme.text, fontWeight: category===cat ? '900' : '600', fontSize:12, marginLeft:8 }}>{cat}</Text>
                                    </TouchableOpacity>
                                ))}
                            </ScrollView>
                        </View>
                    ...
                    ListEmptyComponent={() => !loading ? (
                        <View style={{ alignItems:'center', padding:24 }}>
                            ...
                            <Text style={{ color:theme.textSecondary, fontSize:12, marginTop:8, textAlign:'center' }}>
                                {debouncedSearch.length < 2 && category === 'Todas' ? 'Selecione uma categoria ou busque por nome' : 'Nenhum alimento encontrado'}
                            </Text>
                        </View>
                    ) : null}
                />
...
const styles = StyleSheet.create({
    ...
    catDropList:    { position:'absolute', top:46, left:0, right:0, borderRadius:12, borderWidth:1, zIndex:999, shadowColor:'#000', shadowOffset:{width:0,height:4}, shadowOpacity:0.15, shadowRadius:10, elevation:10 },
    catDropItem:    { flexDirection:'row', alignItems:'center', padding:10, borderBottomWidth:1 },
