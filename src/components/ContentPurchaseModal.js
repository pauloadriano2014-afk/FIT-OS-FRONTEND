// src/components/ContentPurchaseModal.js
// 💳 COMPRA AVULSA DE CONTEÚDO (ebook / audiobook) — chamado a partir do
// modal de upsell da Biblioteca quando o item está bloqueado mas tem `valor`
// definido. Mesmo padrão visual/fluxo do FinancePaymentModal, só que por
// contentId em vez de ciclo de mensalidade.
...
export default function ContentPurchaseModal({ visible, onClose, theme, userId, content, onPurchased }) {
    ...
    const typeLabel = content?.type === 'ebook' ? 'E-BOOK'
        : content?.type === 'audio' ? 'AUDIOBOOK'
        : 'CONTEÚDO';
    ...
    const fetchCheckout = async (cpfToSend = null, isSilent = false) => {
        try {
            ...
            if (data.alreadyOwned || data.paid) {
                setStep('PAID');
                return;
            }
            ...
            console.error('[ContentPurchaseModal] Erro:', err);
            if (!isSilent) {
    ...
    useEffect(() => {
        if (visible && userId && contentId) {
            setStep('LOADING');
            setErrorMsg('');
            setPayment(null);
            fetchCheckout();
        }
    }, [visible, userId, contentId]);

    // 2. SMART POLLING (aguarda o QR PIX gerar)
    useEffect(() => {
        let timeout;
            ...
            <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.85)', justifyContent: 'center', alignItems: 'center' }}>
                <View style={{
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        {step === 'CHARGE' && payment && (
                            <View style={{ alignItems: 'center' }}>
                                <Text style={{ color: theme.textSecondary, fontSize: 11, fontWeight: 'bold', letterSpacing: 1, marginBottom: 4 }}>
                                    {typeLabel} VIP
                                </Text>
                                <Text style={{ color: theme.text, fontSize: 18, fontWeight: '900', textAlign: 'center', marginBottom: 8 }} numberOfLines={2}>
                                    {content?.title}
                                </Text>
                        ...
                        {step === 'PAID' && (
                            <View style={{ alignItems: 'center', paddingVertical: 20 }}>
                                ...
                                <Text style={{ color: theme.textSecondary, fontSize: 13, textAlign: 'center', lineHeight: 20, marginBottom: 20 }}>
                                    {content?.title ? `"${content.title}" já está disponível na sua Biblioteca.` : 'Já está disponível na sua Biblioteca.'}
                                </Text>
                                <TouchableOpacity
                                    style={{ width: '100%', padding: 18, borderRadius: 12, backgroundColor: theme.accent, alignItems: 'center' }}
                                    onPress={() => { if (onPurchased) onPurchased(); onClose(); }}
                                >
                                    <Text style={{ color: '#000', fontWeight: '900', fontSize: 14, letterSpacing: 1 }}>CONTINUAR</Text>
                                </TouchableOpacity>
                            </View>
