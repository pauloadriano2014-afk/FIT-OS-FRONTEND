...
// ─── OPÇÕES DE MÓDULO SAAS ───
const MODULE_OPTIONS = [
    { value: 'TREINO', label: 'Treino', icon: 'dumbbell', color: '#32ADE6' },
    { value: 'DIETA', label: 'Dieta', icon: 'food-apple', color: '#34C759' },
    { value: 'AMBOS', label: 'Treino + Dieta', icon: 'trophy', color: '#FFCC00' },
];
...
export default function AdminUserSystem({
    ...
    studentModules, savingModules, coachPlan, handleSaveStudentModules, isMaster, handleRequestAnamneseUpdate
}) {
    ...
    useEffect(() => {
        ...
        const parts = String(nextCheckInDate).split('/');
        if (parts?.length !== 3) return; // 🔥 PROTEÇÃO
                ...
                <View style={{ flexDirection: isWebPC ? 'row' : 'column', flexWrap: 'wrap', gap: 15 }}>
                    ...
                    <View style={[styles.dashCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                        <View style={styles.dashCardHeader}>
                            <View style={[styles.iconBoxSmall, { backgroundColor: theme.accent + '22' }]}>
                        ...
                        {/* 🔥 PROTEÇÃO: Verificação segura de arrays */}
                    ...
                    <View style={[styles.dashCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                        <View style={styles.dashCardHeader}>
                            <View style={[styles.iconBoxSmall, { backgroundColor: '#34C75922' }]}>
                                <MaterialCommunityIcons name="lightning-bolt" size={16} color="#34C759"/>
                            </View>
                    ...
                    <View style={[styles.dashCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                        <View style={styles.dashCardHeader}>
                            <View style={[styles.iconBoxSmall, { backgroundColor: '#FF3B3022' }]}>
                                <MaterialCommunityIcons name="food-apple" size={16} color="#FF3B30"/>
                            </View>
                ...
                <View style={[styles.premiumCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                    ...
                    <View style={{ padding: 20 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                            ...
                            <TouchableOpacity style={[styles.saveBtnLg, { backgroundColor: theme.accent, width: 54 }]} onPress={handleSaveCheckInDate}>
                ...
                <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border, padding: 15 }]}>
                    ...
                    {evaluationUrl ? (
                        <View style={{ flexDirection: 'row', gap: 10, marginTop: 15 }}>
                            ...
                            <TouchableOpacity style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 10, borderRadius: 8, backgroundColor: 'rgba(255,59,48,0.1)', borderWidth: 1, borderColor: '#FF3B30' }} onPress={() => { setEvaluationUrl(''); const msg = "Link removido. Clique em Salvar para confirmar."; Platform.OS === 'web' ? window.alert(msg) : Alert.alert("Aviso", msg); }}>
                ...
                <View style={[styles.premiumCard, { backgroundColor: theme.surface, borderColor: theme.border, marginTop: 15 }]}>
                    <View style={[styles.cardHeader, { borderBottomColor: theme.border }]}>
                        ...
                        <View style={{ flex: 1 }}>
                            ...
                            <Text style={{ color: theme.textSecondary, fontSize: 11 }}>Sugestão genérica em PDF para alunos sem dieta prescrita.</Text>
                        </View>
                    </View>
                    <View style={{ padding: 20 }}>
    ...
    if (currentTab === 'SISTEMA') {
        const isMasterCoach = isMaster ?? (!aluno?.coachId || ['3c82f763-66b4-48da-836e-16817d4f57c0', 'b7c0c181-41fd-4156-b8fe-963a267759a3'].includes(aluno?.coachId));
        const cPlan = coachPlan || 'ELITE';
        const allowedModules = ALLOWED_BY_PLAN[cPlan] ?? ['TREINO', 'DIETA', 'AMBOS'];

        return (
            <View style={{ paddingBottom: 100 }}>
                
                {/* 🔥 MÓDULOS DO ALUNO (SAAS) - Só aparece para coaches parceiros */}
                {!isMasterCoach && (
                    <View style={{ marginBottom: 20 }}>
                        <Text style={[styles.sectionLabel, { marginTop: 10 }]}>MÓDULOS DO ALUNO (COACH PARCEIRO)</Text>
                        <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border, padding: 15 }]}>
                            <Text style={styles.sectionSubDesc}>Defina o que este aluno tem acesso no aplicativo.</Text>
                                ...
                                {MODULE_OPTIONS.map(opt => {
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            onPress={() => isAllowed && !savingModules && handleSaveStudentModules && handleSaveStudentModules(opt.value)}
                                            disabled={!isAllowed || savingModules}
                                            activeOpacity={0.7}
                                        >
                                            <MaterialCommunityIcons name={opt.icon} size={20} color={isSelected ? opt.color : theme.textSecondary} />
                                            <Text style={{ fontSize: 11, fontWeight: '900', color: isSelected ? opt.color : theme.textSecondary, marginTop: 4, textAlign: 'center' }}>
                                                {opt.label}
                                            </Text>
                                        </TouchableOpacity>
                                    );
                ...
                {/* 🔥 MÓDULOS EXTRAS (CORRIDA E ANAMNESE) */}
                <Text style={[styles.sectionLabel, { marginTop: 10 }]}>MÓDULOS EXTRAS & AÇÕES</Text>
                
                <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border, padding: 15, marginBottom: 15 }]}>
                    <View style={styles.cardHeader}>
                        <View style={[styles.iconBoxSmall, { backgroundColor: theme.accent + '22' }]}>
                            <MaterialCommunityIcons name="run-fast" size={16} color={theme.accent} />
                        </View>
                        <Text style={[styles.dashCardTitle, { color: theme.text }]}>PROTOCOLO DE CORRIDA</Text>
                    </View>
                    <View style={[styles.switchRow, { paddingHorizontal: 0, paddingBottom: 0, borderBottomWidth: 0, marginTop: 10 }]}>
                        <View style={{ flex: 1, paddingRight: 15 }}>
                            <Text style={{ color: theme.text, fontWeight: '700', fontSize: 13 }}>Ativar Corrida</Text>
                            <Text style={{ color: theme.textSecondary, fontSize: 11, marginTop: 2 }}>Libera treinos de corrida para este aluno.</Text>
                        </View>
                        <Switch
                            value={runningModule ?? false}
                            onValueChange={handleToggleRunningModule}
                            trackColor={{ false: theme.border, true: theme.accent + '60' }}
                            thumbColor={(runningModule ?? false) ? theme.accent : theme.textSecondary}
                        />
                    </View>
                </View>

                <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border, padding: 15, marginBottom: 30 }]}>
                    <View style={styles.cardHeader}>
                        <View style={[styles.iconBoxSmall, { backgroundColor: '#BF5AF222' }]}>
                            <MaterialCommunityIcons name="clipboard-text-outline" size={16} color="#BF5AF2" />
                        </View>
                        <Text style={[styles.dashCardTitle, { color: theme.text }]}>DISPARAR ANAMNESE OBRIGATÓRIA</Text>
                    </View>
                    <View style={{ flexDirection: 'row', gap: 10, marginTop: 15 }}>
                        <TouchableOpacity style={[styles.dashActionBtn, { flex: 1, justifyContent: 'center', backgroundColor: theme.bg, borderColor: theme.border }]} onPress={() => handleRequestAnamneseUpdate && handleRequestAnamneseUpdate('TRAINING')}>
                            <Text style={{ color: theme.text, fontSize: 11, fontWeight: 'bold' }}>SÓ TREINO</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.dashActionBtn, { flex: 1, justifyContent: 'center', backgroundColor: '#BF5AF222', borderColor: '#BF5AF2' }]} onPress={() => handleRequestAnamneseUpdate && handleRequestAnamneseUpdate('FULL')}>
                            <Text style={{ color: '#BF5AF2', fontSize: 11, fontWeight: 'bold' }}>TREINO + DIETA</Text>
                        </TouchableOpacity>
                    </View>
                </View>

                {/* 🔥 ZONA DE RISCO (ORIGINAL) */}
...
const styles = StyleSheet.create({
    ...
    cardHeader: { flexDirection: 'row', alignItems: 'center', gap: 15, padding: 0, paddingBottom: 10, borderBottomWidth: 0 },
    ...
    moduleBtn:  { flex: 1, alignItems: 'center', padding: 12, borderRadius: 14, borderWidth: 2, gap: 4 },
});