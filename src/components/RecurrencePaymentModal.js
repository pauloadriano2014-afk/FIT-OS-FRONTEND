// src/components/RecurrencePaymentModal.js
// 💳 Ativa recorrência (cartão salvo, cobrança automática) direto da tela de
// "ACESSO SUSPENSO" — mesmo momento em que o FinancePaymentModal oferece o
// pagamento avulso (PIX/cartão). Mesmo padrão visual/de passos dele, mas
// falando com /api/payments/recurrence/create em vez de /api/payments/checkout.
...
export default function RecurrencePaymentModal({ visible, onClose, theme, userId, onActivated }) {
    ...
    const fetchRecurrence = async (overrides = {}) => {
        try {
            const res = await fetch(`${API_URL}/api/payments/recurrence/create`, {
                method: 'POST',
            ...
            setStep('REDIRECT');
            if (onActivated) onActivated(); // pra já refletir PENDING_CHECKOUT em outras telas
        } catch (err) {
            console.error('[RecurrencePaymentModal] Erro:', err);
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        {step === 'REDIRECT' && (
                            <View style={{ alignItems: 'center', paddingVertical: 10 }}>
                                ...
                                <Text style={{ color: theme.textSecondary, fontSize: 13, textAlign: 'center', lineHeight: 20, marginBottom: 20 }}>
                                    ...
                                    Depois de confirmar lá, seu acesso libera automaticamente.
                                </Text>
                                {checkoutUrl && (
                                    <TouchableOpacity
                        ...
                        {step === 'ALREADY_ACTIVE' && (
                            <View style={{ alignItems: 'center', paddingVertical: 20 }}>
                                ...
                                <Text style={{ color: theme.textSecondary, fontSize: 13, textAlign: 'center', lineHeight: 20, marginBottom: 20 }}>
                                    Seu pagamento automático já está ativado. Se o acesso ainda não liberou, fale com seu coach.
                                </Text>
                                <TouchableOpacity
