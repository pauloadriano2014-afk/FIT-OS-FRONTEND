...
              <View>
                <Text style={[styles.greeting, { color: theme.textSecondary }]}>BEM-VINDO AO {userPlan === 'LOW_COST' ? 'PLANO BÁSICO' : (userPlan === 'FICHA_8S' ? 'PROJETO DE FICHAS' : (userPlan === 'CHALLENGE_21' ? 'DESAFIO 21 DIAS' : 'ELITE'))},</Text>
...
                        <Text style={[styles.xpText, { color: theme.textSecondary }]}>{isFichaPlaceholder ? 'PREPARANDO TREINO' : `SEMANA ${Math.min(8, Math.max(1, Math.ceil(fichaDaysElapsed / 7)))} DE 8`}</Text>
                    </View>
