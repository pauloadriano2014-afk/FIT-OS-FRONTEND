...
export default function AdminUserOptions({ route, navigation }) {
    ...
    // NOVO ESTADO PARA OS CHECKS SEMANAIS
    const [weeklyChecks, setWeeklyChecks] = useState(aluno.weeklyChecks || []);
    const [newCheckText, setNewCheckText] = useState(''); // Para o input de novo check
            ...
            try {
                ...
                if (cached) {
                    ...
                    if (freshness) {
                        ...
                        setWeeklyChecks(freshness.weeklyChecks || []); // Carrega os checks do cache
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            if (resUser.ok) {
                ...
                setWeeklyChecks(fresh.weeklyChecks || []); // Carrega os checks do servidor
    ...
    const handleSaveStrategy = async (newDate = null) => {
        ...
        const payload = {
            strategyNotes,
            weeklyChecks // Adicione weeklyChecks aqui
        };
        if (newDate) payload.lastContactDate = newDate;


        try {
            // ⚠️ ATENÇÃO: Seu back-end precisa aceitar 'strategyNotes' e 'lastContactDate' e 'weeklyChecks' nesta rota!
    ...
    // Funções para gerenciar os checks semanais
                        ...
                        <View style={[styles.dashCard, { backgroundColor: theme.surface, borderColor: isContactDelayed ? '#FF3B30' : theme.accent, marginBottom: 20, borderWidth: 2 }]}>
                            ...
                            <View style={{ marginTop: 15, marginBottom: 15 }}>
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10 }}>
                                    <TextInput
                                        style={[styles.inputArea, { flex: 1, minHeight: 40, paddingVertical: 8, paddingHorizontal: 10, marginRight: 10 }]}
                                        placeholder="Adicionar novo ponto de alinhamento..."
                                        placeholderTextColor={theme.textSecondary}
                                        value={newCheckText}
                                        onChangeText={setNewCheckText}
                                        onSubmitEditing={handleAddCheck} // Adiciona ao pressionar Enter
                                    />
                                    <TouchableOpacity
                    ...
                    <View style={[styles.menuModalContent, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                        {MENU_TABS.map((t, index) => (
                            <TouchableOpacity
                                ...
                                onPress={() => {
                                    setActiveTab(t.id); setIsMenuVisible(false);
                                }}
                            >
