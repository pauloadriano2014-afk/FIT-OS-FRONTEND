...
import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, Platform, KeyboardAvoidingView, useWindowDimensions, ActivityIndicator, Animated, Alert } from 'react-native';
...
export default function AdminDietScreen({ route, navigation }) {
    ...
    const handleSaveDiet = async () => {
        ...
        try {
            const safeMeals = actions.meals.map(m => ({ ...m, dayType: (m.dayType === 'PADRÃO' || !m.dayType) ? 'TREINO' : m.dayType }));
            const payload = { userId, name: `Plano Alimentar - ${data.dietConfig.goal}`, goal: data.dietConfig.goal, totalKcal: actions.currentMacros.kcal, totalProtein: actions.currentMacros.prot, totalCarbs: actions.currentMacros.carb, totalFats: actions.currentMacros.fat, waterIntake: data.dietConfig.water, generalNotes: data.dietConfig.notes, meals: safeMeals };
            await DietService.saveDiet(payload);
            if (isWeb) window.alert("🚀 Dieta salva nas abas com sucesso!"); else Alert.alert("Sucesso", "Dieta salva!");
        } catch (error) { Alert.alert("Erro Técnico", error.message); } finally { data.setIsSaving(false); }
    };

    const handleGenerateAI = async () => {
        if (!data.anamnese || Object.keys(data.anamnese).length === 0) return isWeb ? window.alert("Preencha a anamnese primeiro.") : Alert.alert("Atenção", "Preencha a anamnese primeiro.");
        data.setIsGenerating(true);
        try {
            const res = await DietService.generateAIDiet(data.anamnese);
            if (res.meals && res.meals.length > 0) {
                const newMeals = enrichMealsWithDatabase(res.meals).map(m => ({...m, dayType: 'TREINO'}));
                actions.setMeals(newMeals);
                actions.setActiveDayType('TREINO');
                if (isWeb) window.alert("Estratégia Pronta!"); else Alert.alert("Estratégia Pronta!", "Revise e guarde!");
            }
        } catch (error) { Alert.alert("Erro", "Falha na IA."); } finally { data.setIsGenerating(false); }
    };
    ...
    const handleCloneFromStudent = async (sourceStudentId) => {
        ...
        } catch (e) { Alert.alert("Erro", "Falha ao clonar."); }
    };
    ...
    const handleApplyMealTemplate = (template) => {
        ...
        actions.setMeals(prev => prev.map(m => m.id === actions.selectedMealForAction.id ? { ...m, name: template.category || m.name, items: parsedItems } : m));
    ...
    const handleCallGeneratePDF = () => {
        generateDietPDF({ visibleMeals: actions.visibleMeals, dietConfig: data.dietConfig, currentMacros: actions.currentMacros, activeDayType: actions.activeDayType, aluno });
    };
            ...
            <View style={{ flex: 1, width: '100%', maxWidth: containerMaxWidth, alignSelf: 'center', backgroundColor: theme.bg, ...containerBorders }}>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }} enabled={!isWeb}>
                    <Animated.ScrollView style={{ flex: 1, opacity: fadeAnim }} contentContainerStyle={{ paddingBottom: 140 }} keyboardShouldPersistTaps="handled">
                        ...
                        <DietHeaderWidgets theme={theme} currentMacros={actions.currentMacros} macros={actions.macros} pct={pct} showRaioX={showRaioX} setShowRaioX={setShowRaioX} anamnese={data.anamnese} handleGenerateAI={handleGenerateAI} isGenerating={data.isGenerating} setImportModalVisible={modals.setImportModalVisible} />
                        ...
                        <View style={{ paddingHorizontal: 16 }}>
                            <View style={styles.daysTabsContainer}>
                                {['TREINO', 'CARDIO', 'DESCANSO'].map(type => (
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            styles.dayTab,
                                            actions.activeDayType === type
                                                ? { backgroundColor: theme.accent, borderColor: theme.accent, elevation: 4 }
                                                : { backgroundColor: theme.surface, borderColor: theme.border }
                                        ]}
                                        onPress={() => {
                                            Haptics.selectionAsync();
                                            actions.setActiveDayType(type);
                                        }}
                                    >
                                        <Text style={{ color: actions.activeDayType === type ? '#000' : theme.textSecondary, fontWeight: '900', letterSpacing: 0.5, fontSize: 10 }}>
                                            DIA DE {type}
                                        </Text>
                                    </TouchableOpacity>
                                ))}
                            ...
                            {actions.visibleMeals.length === 0 ? (
                                <View style={[styles.emptyBox, { borderColor: theme.border }]}>
                                    ...
                                    <Text style={[styles.emptyDesc, { color: theme.textSecondary }]}>Adicione refeições para o dia de "{actions.activeDayType}".</Text>
                                </View>
                            ) : (
                                actions.visibleMeals.map(meal => (
                                    <MealCardAdmin
                                        key={meal.id} meal={meal} index={actions.visibleMeals.indexOf(meal)} totalMeals={actions.visibleMeals.length}
                                        theme={theme} toGrams={toGrams} handleOpenNameSelect={handleOpenNameSelect}
                                        handleOpenTimeSelect={handleOpenTimeSelect} handleDeleteMeal={actions.handleDeleteMeal}
                                        handleMoveMeal={actions.handleMoveMeal} handleUpdateFoodAmount={actions.handleUpdateFoodAmount}
                                        handleToggleUnit={actions.handleToggleUnit} handleDeleteFood={actions.handleDeleteFood}
                                        handleOpenSearch={handleOpenSearch} handleMealOptions={handleMealOptions}
                                        handleSwapBaseFood={handleSwapBaseFood} handleUpdateMeal={actions.handleUpdateMeal}
                                    />
                                ))
                            )}

                            <View style={{ flexDirection: 'row', gap: 12, marginTop: 20 }}>
                                <TouchableOpacity style={[styles.addMealBtn, { flex: 1, borderColor: theme.accent + '50', backgroundColor: theme.accent + '08' }]} onPress={() => handleActionPress(actions.handleAddMeal)} activeOpacity={0.75}>
                                ...
                                <TouchableOpacity style={[styles.addMealBtn, { flex: 1, borderColor: '#FF3B3050', backgroundColor: '#FF3B3008' }]} onPress={() => handleActionPress(actions.handleClearDay)} activeOpacity={0.75}>
...
const styles = StyleSheet.create({
    ...
    daysTabsContainer: { flexDirection: 'row', gap: 10, marginBottom: 20 },
    dayTab: { flex: 1, paddingVertical: 12, borderRadius: 16, alignItems: 'center', justifyContent: 'center', borderWidth: 1 },
