...
export default function FinishScreen({ route, navigation }) {
        ...
        if (preloadedFile && navigator.canShare && navigator.canShare({ files: [preloadedFile] })) {
            try {
                await navigator.share({
                    files: [preloadedFile],
                    title: 'Treino Pago!',
                    text: `🔥 Treino ${workoutName || 'do dia'} pago na consultoria ELITE FIT!\n💪 Intensidade: ${rpeLabel || 'MÁXIMA'}\n\nFaça parte da Elite!`
                });
            } catch (shareError) {
            ...
            if (navigator.share) {
                ...
                window.alert("📸 Tire um print (screenshot) do seu card para postar e marque o ELITE FIT!");
            }
        }
      } else {
  ...
  const CardContent = () => (
    <View nativeID="share-card-web" style={styles.shareCard}>
      ...
      <Text style={styles.brandFooter}>ELITE FIT</Text>
    </View>
  );
