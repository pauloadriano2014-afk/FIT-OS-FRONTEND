...
// 🔥 LÓGICA FINANCEIRA CENTRALIZADA (mesma usada na Home e nos Treinos)
...
export default function DietScreen({ route }) {
    ...
    // 🔥 FINANCEIRO + CLAIM DE PAGAMENTO — centralizado, igual à Home e Treinos
    ...
    useEffect(() => {
        const initialize = async () => {
            try {
                ...
                // 🔥 CHECAGEM FINANCEIRA — busca sempre a versão mais atual do
                // backend (inclui o estado do claim "Já paguei"), em vez de
                // confiar só no objeto local vindo de route.params/AsyncStorage.
                const financeResult = await finance.fetchFinanceStatus(u.id);
    ...
    // 🔥 "Já paguei" — se der certo, recarrega a tela inteira (libera o cardápio)
    ...
    if (!loading && finance.isFinanceLocked) {
        return (
            <RootComponent style={[styles.centered, { backgroundColor: theme.bg }]}>
                ...
                <Text style={[styles.stateDesc, { color: theme.textSecondary, marginBottom: 20 }]}>
                ...
                <View style={[styles.topHeader, { backgroundColor: theme.bg }]}>
                    ...
                    {finance.isPaymentClaimActive && (
                        <View style={[styles.claimReviewBanner, { backgroundColor: '#32ADE622', borderColor: '#32ADE6' }]}>
                            <MaterialCommunityIcons name="clock-check-outline" size={20} color="#32ADE6" />
                            <View style={{ flex: 1, marginLeft: 8 }}>
                                <Text style={{ color: '#32ADE6', fontWeight: '900', fontSize: 11, letterSpacing: 0.3 }}>
                                    PAGAMENTO EM ANÁLISE
                                </Text>
                                <Text style={{ color: theme.textSecondary, fontSize: 10, marginTop: 2 }}>
                                    Liberado enquanto seu coach confirma{finance.paymentClaimDaysLeft != null ? ` (até ${finance.paymentClaimDaysLeft} dia${finance.paymentClaimDaysLeft === 1 ? '' : 's'})` : ''}.
                                </Text>
                            </View>
                        </View>
                    )}
...
const styles = StyleSheet.create({
    ...
    // 🔥 Banner de pagamento em análise + expirado
    claimReviewBanner: { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 12, borderWidth: 1, marginBottom: 15 },
