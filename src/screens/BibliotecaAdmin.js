...
export default function BibliotecaAdmin({ navigation }) {
  ...
  const renderSection = ({ item }) => (
    <View style={styles.section}>
      <ImageBackground
        source={{ uri: categoryCovers[item.category] || categoryCovers.TODOS }}
        style={styles.categoryCover}
        imageStyle={{ borderRadius: 20, resizeMode: 'contain' }}
      >
        <View style={styles.coverOverlay}>
          <Text style={styles.coverTitle}>{item.category.toUpperCase()}</Text>
...
const styles = StyleSheet.create({
  ...
  catScroll: { marginBottom: 10, paddingHorizontal: 20 },
  catTab: {
  ...
  categoryCover: { height: 200, width: '100%' },
  coverOverlay: {
  ...
  coverCount: { color: '#CCFF00', fontSize: 10, fontWeight: 'bold' },
